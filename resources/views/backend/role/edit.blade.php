...
<div class="role-form-page">
    <div class="role-form-hero">
        <div>
            ...
            <h1>{{ $title }}</h1>
            <p>Perbarui nama role dan sinkronkan permission granular agar akses backend tetap sesuai kebutuhan operasional.</p>
        </div>
                ...
                <div class="role-form-panel">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $role->name) }}" placeholder="Contoh: auditor-desa" required>
                            @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <small class="form-text text-muted">Role `super-admin` dikunci dari halaman edit karena memiliki bypass akses penuh.</small>
                    </div>
                            ...
                            <div class="permission-card" data-permission-group="{{ strtolower($group['name']) }}">
                                ...
                                <div class="permission-list">
                                    @foreach($group['permissions'] as $permission)
                                        @php
                                            $checked = is_array($oldPermissions)
                                                ? in_array($permission['name'], $oldPermissions, true)
                                        ...
                                        <label class="permission-item" data-permission-name="{{ strtolower($permission['name']) }}" for="perm_{{ $permission['id'] }}">
                                            <input type="checkbox" name="permissions[]" id="perm_{{ $permission['id'] }}" value="{{ $permission['name'] }}" {{ $checked ? 'checked' : '' }}>
